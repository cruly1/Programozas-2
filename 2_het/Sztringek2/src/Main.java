public class Main {
    ...
    private static String verbing(String s) {
        if (s.length()<3) {
            return s;
        } else if (s.substring(s.length()-3).equals("ing")) {
            return s + "ly";
        }
    ...
    private static String frontBack(String a, String b) {
        int kellEzASzar = a.length() / 2 + a.length() % 2;
        int megEzASzarIs = b.length() / 2 + b.length() % 2;

        String aFront = a.substring(0, kellEzASzar);
        String bFront = b.substring(0, megEzASzarIs);
        String aBack = a.substring(kellEzASzar);
        String bBack = b.substring(megEzASzarIs);
